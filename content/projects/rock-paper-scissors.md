...
- [Tailwind (RU дока)](https://tailwindcss.ru/) - это утилитарный CSS-фреймворк, где стили задаются через готовые классы прямо в HTML. Вместо написания кастомного CSS, разработчики используют классы, такие как bg-blue-500 или p-4, для быстрой и гибкой стилизации. Tailwind ускоряет работу и упрощает создание адаптивных интерфейсов.
  - [Как установить Tailwind CSS в проекте с Vite](https://tailwindcss.com/docs/installation/using-vite)
  - [Основные концепции](https://tailwindcss.ru/docs/utility-first/). Тот редкий случай, когда официальная документация написана простым языком.
  - Когда дойдете до практики, то рекомендую пользоваться [английской документацией](https://tailwindcss.com/docs/display), т.к. большинство tailwind классов названы идентично CSS свойствам, а в русской версии их зачем-то перевели. Отсюда может возникнуть путаница и недопонимание в будущем, т.к. русскоговорящие разработчики используют оригинальные названия CSS свойств, а не переведенные.
...
1. Создайте новое React приложение с помощью vite следующей командой
```bash
npm create vite@latest rock-papper-scissors -- --template react
```
2. Для стилизации должен быть использован фреймворк tailwind 
